...
// Code adapted from Scilab src/c/dde.c
const char *SumatraFwd(char *execString){
    ...
    if(DdeInitialize(&idInst, (PFNCALLBACK)DdeCallback, APPCLASS_STANDARD | APPCMD_CLIENTONLY, 0 ) == DMLERR_NO_ERROR){
        strcpy(Reply, "DDE error initializing");
        return(Reply);
    }

    hszApp = DdeCreateStringHandle(idInst, "SUMATRA", 0);
    hszTopic = DdeCreateStringHandle(idInst, "control", 0);
    hConv = DdeConnect(idInst, hszApp, hszTopic, NULL);
    DdeFreeStringHandle(idInst, hszApp);
    DdeFreeStringHandle(idInst, hszTopic);

    if(hConv == NULL){
        strcpy(Reply, "DDE error connecting");
        DdeUninitialize(idInst);
        return(Reply);
    }

    HDDEDATA hData = DdeCreateDataHandle(idInst, (LPBYTE)execString, lstrlen(execString)+1, 0, NULL, CF_TEXT, 0);

    if(hData==NULL) {
        strcpy(Reply, "DDE error seting data");
    } else {
        if(DdeClientTransaction((LPBYTE)hData, 0xFFFFFFFF, hConv, 0L, 0, XTYP_EXECUTE, TIMEOUT_ASYNC, NULL) == NULL)
            strcpy(Reply, "DDE error sending message");
    }
